...
export class DeepSeekAdapter implements AIAdapterInterface {
  ...
  async classify(model: string, title: string, content: string, apiKey: string, baseUrl?: string): Promise<string> {
    const result = await callOpenAICompatibleAPI(
      ...
      [
        { role: 'system', content: '你是一个日志分类助手。请根据标题和内容将日志分类为：meeting(会议)、task(任务)、chore(杂务)、personal(个人)、work(工作)、study(学习)、health(健康)、other(其他)。只回复类别英文名，不要其他内容。' },
  ...
  async summary(model: string, type: string, date: string, logs: Log[], apiKey: string, baseUrl?: string): Promise<string> {
    ...
    return callOpenAICompatibleAPI(
      baseUrl || DEFAULT_ENDPOINT, apiKey, model,
      [
        { role: 'system', content: `你是一个日志总结助手。请根据提供的日志数据生成${label}总结。要求：1.概括主要事项 2.分析完成情况 3.给出改进建议 4.统计各分类占比。请用中文输出，格式清晰。` },
